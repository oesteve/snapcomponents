...
import {type FormElementProps, useFieldData, useSetFieldValue} from "@/components/form";
...
export function IntentsWidget(props: FormElementProps) {
    ...
    function handleRemove(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, idx) {
        e.preventDefault()
        e.stopPropagation()
        setFieldValue(`${props.name}[${idx}]`, undefined)
    }

    function handleEdit(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, toEdit: string) {
        e.preventDefault()
        e.stopPropagation()
        setToEdit(toEdit)
    }
    ...
    return (
        <div className="space-y-2">
            ...
            {toEdit && <EditIntentDialog name={toEdit} onClosed={() => setToEdit(undefined)}/>}
        </div>
    )
}
